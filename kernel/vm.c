...
int mapSuperPage(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  for (;;)
  {
    ...
    {
      printf("walkSuper error \n");
      return -1;
    }
    *pte = PA2PTE(pa) | perm | PTE_U | PTE_V | PTE_SUPER;
    if (a == last)
      break;
    a += SUPERPGSIZE;
    pa += SUPERPGSIZE;
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  for (a = oldsz; a < newsz; a += sz)
  {
    ...
    if (sz == SUPERPGSIZE)
      printf("uvmalloc SUPERPGSIZE\n");

    if (mem == 0)
    {
      printf("unmapped: mem == 0\n");
...
int uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for (i = 0; i < sz; i += szinc)
  {
    ...
    else
    {
      ...
      if (mapSuperPage(new, i, SUPERPGSIZE, (uint64)mem, flags) != 0)
      {
        superKfree(mem);
        goto err;
      }
...
#ifdef LAB_PGTBL
// solution: vmprint()
void recursive_vmprint(pagetable_t pagetable, uint64 depth)
{
